...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/22 04:21:31 by agurses           #+#    #+#             */
/*   Updated: 2025/08/22 04:21:32 by agurses          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

int	fill_philosophers(t_philo **philosophers, int philo_count)
{
	int	i;
	...
	while (i < philo_count)
	{
		...
		(*philosophers)[i].right_fork = &((*philosophers)->data->forks[(i + 1)
				% philo_count]);
...
int	init_forks(t_philo **philosophers, char **argv)
{
	...
	i = -1;
	(*philosophers)->data->forks = ft_calloc(ft_atoi(argv[1]),
...
int	init_philosophers(char **argv, t_philo **philosophers)
{
	...
	*philosophers = ft_calloc(ft_atoi(argv[1]), sizeof(t_philo));
	...
	if (fill_philosophers(philosophers, ft_atoi(argv[1])) == 1)
	{
		free((*philosophers)->data);
		free(*philosophers);
		return (1);
