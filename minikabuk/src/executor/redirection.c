...
char **extract_clean_command(t_token_list *token_list)
{
    ...
    int count = 0;
    int i = 0;
    int skip_next = 0; // Bir sonraki tokeni atlamak için flag
    
    tmp = token_list;
    
    // Önce sadece COMMAND ve WORD tipindeki token sayısını hesapla
    while (tmp)
    {
        if (skip_next)
        {
        ...
        {
            skip_next = 1; // Bir sonraki tokeni (dosya adını) atla
        }
        tmp = tmp->next;
    }
    
    // Komut argümanlarını kopyala
...
int setup_heredoc(char *delimiter)
{
    ...
    return (pipe_fd[0]); // Okuma ucunu döndür
}
...
int handle_redirect_or_heredoc(t_minishell *minishell, t_token_list **token_list)
{
    ...
    // Redirect bilgilerini saklamak için değişkenler
    ...
    // Orijinal fd'leri kaydet
    saved_stdin = dup(STDIN_FILENO);
    saved_stdout = dup(STDOUT_FILENO);

    // Token listesini tara ve redirect bilgilerini topla
    while (tmp)
    {
        if (tmp->token->type == TOKEN_REDIRECT_IN && tmp->next)
        {
            input_file = tmp->next->token->value;
            tmp = tmp->next; // Dosya adını atla
        }
...
int execute_redirect_herodoc_child(t_minishell *minishell)
{
	...
	else
	{
		...
		if (pid == 0)
		{
		    ...
		    // execve başarılı olursa buraya hiç gelmez
		}
		else if (pid > 0)
		{
