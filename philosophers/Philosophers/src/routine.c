...
void	philo_eat(t_philo *philo)
{
	if (one_philo(philo) == 1)
		return;
	//sem_wait(&philo->data->waiter); // MASA SEMAFORU: izin al
	if (philo->id % 2 == 0)
	{
	...
	//sem_post(&philo->data->waiter); // MASA SEMAFORU: izin bırak
}
...
int start_simulation(t_philo *philosophers)
{
	...
	// delayed start yöntemi: thread'ler başlatıldıktan sonra senkronize başlangıç
	start_time = get_time() + (count * 50); // Tüm thread'lerin başlaması için zaman tanı

	// Tüm filozoflara aynı start_time'ı ata
	i = 0;
	while (i < count)
	{
