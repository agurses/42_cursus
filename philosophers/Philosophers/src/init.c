...
void	fill_philosophers(t_philo ** philosophers, int philo_count)
{
	...
	while (i < philo_count)
    {
		...
		pthread_mutex_init(&(*philosophers)[i].meal_time_lock, NULL);
		///// BURADA FREE LER VE HATA KONTROLÜ YOK
		i++;
	}
}
...
int	init_data(t_philo **philosophers, char **argv)
{
	...
	if (init_forks(philosophers, argv))
		return (-1);
	//sem_init(&(*philosophers)->data->waiter, 0, (*philosophers)->data->num_philosophers.num_philosophers - 1);
	return (0);
}
