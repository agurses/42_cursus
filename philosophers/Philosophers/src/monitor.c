...
void	*death_monitor(void *arg)
{
	...
	while (1)
	{
		...
		// Ölüm kontrolü : her filozof için ayrı lock
		while (++i < count)
		{
		...
		// Herkes yeterince yediyse programı bitir
		if (philo->data->number_of_times_each_philosopher_must_eat != -1 
			&& finished_eating >= count)
		{
			pthread_mutex_lock(&philo->data->death_mutex);
			philo->data->any_philosopher_died = 1; // Programı sonlandırmak için
			pthread_mutex_unlock(&philo->data->death_mutex);
			return (NULL);
		}
		
		usleep(1000); // Orjinal değer
	}
	return (NULL);
}
